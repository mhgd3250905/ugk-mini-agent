...
- **主题**: Team Console Execution Atlas 框选和 UI-only Group 交互优化。框选后节点高亮增强，深色模式下选中态更明显；点击已选节点集合外的空白或其他节点会清空框选，点击/拖动已选节点保留多选。折叠 Group 可拖动并在右侧显示 Task 数量；展开 Group 支持上锁/解锁和移除，锁定后 Group 及内部 Task 都不能移动，也不能删除 Group。
