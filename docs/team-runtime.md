...
> 2026-06-04 补充：Team Console Step 03 已增加 Task 级历史 run 装载 UI 状态。运行记录行提供“装载此记录”/“取消装载”，可见标记为“已装载”；若同一 Task 有 queued/running active run，标记降级为“已装载（活跃 run 优先）”，只保留引用而不把历史 run 展示成当前活跃上下文。该状态只在 Team Console `canvas-ui-state` 中保存 `loadedTaskRunSelections: Array<{ taskId, runId }>`，不保存 artifact/content/attempt/files，也不改变 `POST /v1/team/tasks/:taskId/runs` 请求；`upstreamRunSelections` 接线留给后续 Step 04。
