...
- P21-B 只提供 schema、runner contract 和持久化记录；`TeamOrchestrator` 尚不会调用 decomposer，也不会生成/执行 child task

语义边界：

- `none` — 当前任务不可拆分
- `leaf` — 后续 P21-C 中可拆成普通 child task；child task 不再继续拆
- `propagate` — 后续 P21-C 中可拆成 `none | leaf` child task；首版不允许继续生成 `propagate`
...
| `src/team/run-workspace.ts` | `writeExpansion` / `readExpansion` / `appendChildTaskStates`，以及 P21-B 的 `writeDecomposition` / `readDecomposition` 持久化方法 |
...
当前限制：P21-B 不会在 orchestrator 中调用 `runDecomposer()`，所以不会真正拆分任务。
...
```text
runs/<runId>/decompositions/<parentTaskId>.json
```
...
记录必须保留完整 child `TeamTask` 定义，确保后续 P21-C resume/reclaim 不会丢失 child input/acceptance/decomposer 策略。旧 run 没有 `decompositions/` 目录时 `readDecomposition()` 返回 `null`，不影响 state/attempt 读取。
...
8. **P21-B 不执行 decomposition** — task decomposer schema、runner contract 和 decomposition record 已就绪，但 orchestrator 尚不会调用 `runDecomposer()`；运行时拆分、child task state 和父子状态推导留到 P21-C。
