...
- Team Console preview 的 Agent 能力已收口为 Agent Atlas：通过 `GET /v1/agents` 读取主项目 Agent catalog，Agent 节点加入同一张 Execution Atlas，复用网格、节点样式、pan/zoom 和“重置视图”；默认 Mock 入口是干净 `Agent workspace`，不显示旧 demo run。画布内同一 `agentId` 只能出现一次；普通画布态可拖拽 Agent 卡片，拖拽只改变 Team Console 画布引用位置，不修改真实 Agent profile，也暂不持久化。单击 Agent 节点会进入 Focus Mode 特殊 Agent 对话界面：普通节点层、其他 Agent、runtime nodes、links、evidence、添加入口和缩放工具隐藏，只显示当前 Agent 卡片和下方大对话工作区；Focus 只能通过“收起”退出，退出后恢复进入 Focus 前的 viewport 和普通画布。Focus 不是轻量 chat panel，而是画布内固定 Agent 的主对话替代入口：顶部保留新会话、文件库和上下文使用量入口，但隐藏 Agent switcher / Agent 切换按钮；后台任务和 Team Runtime 入口暂不显示，避免在 Agent 对话场景里扩散额外工作台功能。composer 保留左侧文件选择、已选文件移除、上传中状态、发送和运行中打断能力，不显示 `Shift+Enter 换行` 可见提示；只选择文件不输入文本时会补默认请求文案，避免向真实 chat 接口发送空 `message`。Live scoped chat 仍是非 stream `POST /v1/agents/:agentId/chat`，并复用返回的 `conversationId` 继续同一 Agent 会话；文件库和上传走 `/v1/assets`，上下文状态优先读 `/v1/agents/:agentId/chat/status`，打断只在已有 `conversationId` 后启用。该能力只引用现有 Agent profile，不创建 clone、instance、overlay 或画布局部技能安装，也不落地 WorkUnit / Plan 编排；仍不接 artifact preview / event stream。本轮不处理移动端 toolbar / 添加入口专项修复。
