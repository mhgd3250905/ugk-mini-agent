...
- Team Console preview 的 Agent 能力已收口为 Agent Atlas：通过 `GET /v1/agents` 读取主项目 Agent catalog，Agent 节点加入同一张 Execution Atlas，复用网格、节点样式、pan/zoom 和“重置视图”；默认 Mock 入口是干净 `Agent workspace`，不显示旧 demo run。画布内同一 `agentId` 只能出现一次；普通画布态可拖拽 Agent 卡片，拖拽只改变 Team Console 画布引用位置，不修改真实 Agent profile，也暂不持久化。单击 Agent 节点会进入 Focus Mode 特殊 Agent 对话界面：普通节点层、其他 Agent、runtime nodes、links、evidence、添加入口和缩放工具隐藏，只显示当前 Agent 卡片和下方大对话工作区；Focus 只能通过“收起”退出，退出后恢复进入 Focus 前的 viewport 和普通画布。Focus 不是轻量 chat panel，而是画布内固定 Agent 的主对话替代入口：顶部保留新会话、文件库和上下文使用量入口，但隐藏 Agent switcher / Agent 切换按钮；仍不恢复后台任务或 Team Runtime 入口，避免在 Agent 对话场景里扩散额外工作台功能。composer 保留左侧文件选择、已选文件移除、上传中状态、发送和运行中打断能力，不显示 `Shift+Enter 换行` 可见提示；只选择文件不输入文本时会补默认请求文案，避免向真实 chat 接口发送空 `message`。Live scoped chat 通过 `/v1/agents/:agentId/chat/stream` 发送并接收 `run_started`、`text_delta`、`done`、`error`、`interrupted` 与 `queue_updated`；进入 Focus 时通过 `/v1/agents/:agentId/chat/conversations` 找 current conversation，并通过 `/v1/agents/:agentId/chat/state` 恢复当前会话状态、history view 和 active run，后续历史分页边界沿用 `/v1/agents/:agentId/chat/history`。运行中再次发送走 `/v1/agents/:agentId/chat/queue`，打断走 `/v1/agents/:agentId/chat/interrupt`；文件库和上传走 `/v1/assets`，上传会携带已有 `conversationId`。该能力只引用现有 Agent profile，不创建 clone、instance、overlay 或画布局部技能安装，仍不落地 WorkUnit / Plan 编排；仍不接 artifact preview，不处理移动端 toolbar / 添加入口专项修复。
...
- Execution Atlas 桌面画布支持鼠标滚轮缩放、背景拖拽平移和“放大 / 缩小 / 重置视图”工具按钮；pan/zoom 只是本地 UI 状态，不持久化。Evidence / preview 高度测量使用 transform-independent `offsetHeight` 优先，避免缩放后把 `getBoundingClientRect().height` 写回 layout 造成测量反馈循环；滚轮缩放使用原生 non-passive `wheel` listener。移动端本轮不做深度设计，`720px` 以下仍走纵向流式布局并隐藏自定义 pan/zoom 工具条，只保证不明显横向炸版。
