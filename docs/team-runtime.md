...
- Team Console Task run observer 已新增 `Worker 过程` / `Checker 过程` 独立 canvas branch node。前端消费 Canvas Task run attempt metadata 的 additive frontend contract：`roleProcesses.worker` / `roleProcesses.checker`；旧 Live API attempt 缺少 `roleProcesses` 时显示等待过程数据，role process 为 `null` 或条目为空时显示暂无过程条目。过程节点顶部显示角色状态、current action 和最新 narration，下半部按 `toolCallId` 分组展示 `AgentChatProcessEntry`，可折叠 / 展开；这些节点参与现有 Task 操作树拖动语义。当前仍使用现有 run state / attempt metadata / attempt file API 轮询，不接 SSE，不新增 endpoint，不改后端过程存储。
...
- Run observer 还会为 worker / checker 渲染 `Worker 过程` / `Checker 过程` 节点：读取 `attempt.roleProcesses.worker` / `attempt.roleProcesses.checker`，显示状态、current action、最新 narration，并按 `toolCallId` 折叠展示 tool entries。缺少 `roleProcesses` 时前端保持兼容渲染等待态，不影响 Run 状态、文件节点和文件详情。
