...
- Team Console preview 的 Agent 能力已收口为 Agent Atlas：通过 `GET /v1/agents` 读取主项目 Agent catalog，Agent 节点加入同一张 Execution Atlas，复用网格、节点样式、pan/zoom 和“重置视图”；默认 Mock 入口是干净 `Agent workspace`，不显示旧 demo run。画布内同一 `agentId` 只能出现一次；普通画布态可拖拽 Agent 卡片，拖拽只改变 Team Console 画布引用位置，不修改真实 Agent profile，也暂不持久化。单击 Agent 节点在同一 viewport 内进入 Focus Mode，并在节点下方展开非 stream `POST /v1/agents/:agentId/chat` 聊天面板；Focus Mode 是固定锁定视窗，禁用 zoom、pan、工具条缩放 / 重置和 Agent 拖拽，收起恢复进入 Focus Mode 前的 viewport。Live scoped chat 会复用返回的 `conversationId` 继续同一 Agent 会话。该能力只引用现有 Agent profile，不创建 clone、instance、overlay 或画布局部技能安装，也不落地 WorkUnit / Plan 编排。本轮不处理移动端 toolbar / 添加入口专项修复。
