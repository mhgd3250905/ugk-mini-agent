...
| `src/team/orchestrator.ts` | 按 task type 分发执行：normal / discovery / for_each；处理 controlled decomposition；`TaskExpansionPlanner` 和 child execution module 由 orchestrator 组合 |
...
| `src/team/orchestrator.ts` | run 创建、状态迁移、worker/checker/watcher/finalizer 编排（含 discovery / for_each 动态扩展、controlled decomposition 和 output validation gate） |
...
| `src/team/run-workspace.ts` | run 目录、state、attempt、resultRef、final-report、attempt 文件读取、role workspace 安全读取、expansion / decomposition 持久化 |
