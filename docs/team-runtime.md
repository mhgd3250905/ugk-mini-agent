...
- Team Console preview 现在会消费 `GET /v1/team/tasks` 作为 Task catalog：Task 内部包含一个 WorkUnit，Atlas Task 卡片展示 leader Agent、worker Agent 和 checker Agent。点击 Task 后先展开操作菜单；“运行”仍是未接线 disabled 占位，“编辑”只允许浅改 Task 名称、leader Agent、worker Agent、checker Agent，“对话 Leader”打开 `/playground?view=chat&agentId=<leaderAgentId>&embed=team-console&teamTaskId=<taskId>&teamTaskMode=edit` iframe，“删除”二次确认后调用 `POST /v1/team/tasks/:taskId/archive` 软归档并刷新。Live API 工具栏的“创建 Task”只负责选择 leader Agent 并打开 `/playground?view=chat&agentId=<leaderAgentId>&embed=team-console&teamTaskMode=create` iframe；真正创建和复杂 WorkUnit 更新仍由 `/team-task` skill 完成。Team Console 不解析 iframe 聊天文本创建 Task，不把 Task 定义写入 localStorage，也不启动 Task run；Live API 下只持久化 Task 卡片的画布位置，手动刷新、关闭创建分支、浅编辑保存和归档成功后会重新请求 `GET /v1/team/tasks`。
...
- 点击已有 Task 先打开操作菜单：运行是 disabled 占位；编辑只开放 Task 名称、leader Agent、worker Agent、checker Agent；对话 Leader 打开 leader Agent 的 `/playground?view=chat&agentId=<leaderAgentId>&embed=team-console&teamTaskId=<taskId>&teamTaskMode=edit` iframe；删除调用 archive 软归档。
- 浅编辑保存时，标题和 leader Agent 只发送顶层字段；worker/checker 变更会发送现有 `workUnit` 的副本并只替换 Agent 绑定。input text、output contract、acceptance rules 等复杂 WorkUnit 字段不进入 Team Console 可视化编辑 UI。
