...
- `forEach.taskTemplate.decomposer` 在 Plan 创建时被拒绝——parallel child 不允许再拆分
...
- `forEach.mode = "parallel"` + `forEach.taskTemplate.decomposer` 在 Plan 创建时被拒绝——parallel 子任务不允许再拆分
