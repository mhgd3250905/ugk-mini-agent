...
- Canvas Task run 过程观测已接入：worker/checker 的 Agent session raw events 复用主 chat 的 `createAgentSessionEventAdapter()` 和 `applyChatStreamEventToActiveRunView()` 投影为 `ChatProcessBody`，并把 Agent 自述文本写入 `assistantText`；这些数据写入 attempt metadata 的 `roleProcesses.worker` / `roleProcesses.checker`，前端通过现有 attempts API 轮询读取。
...
- attempt metadata 可包含 `roleProcesses.worker` / `roleProcesses.checker`，每个 role process 记录 `role`、`profileId`、`status`、`startedAt`、`updatedAt`、`finishedAt`、`process`，以及可选 `assistantText`
- `assistantText.content` 来自 `message_update/text_delta` 累积出的 active run text，用于 Team Console 展示类似主 `/playground` 的 Agent 自述；工具事件继续留在 `process.entries` 里作为可展开证据
...
- 单条 process entry 的 `detail` 和 `assistantText.content` 持久化前截断到约 8,000 字符并追加 `...[truncated]`，长输出继续以 attempt 文件为准
- role process 进入 `succeeded` / `failed` / `cancelled` 后会忽略迟到 raw session event，避免终态和最终 `assistantText` 被覆盖
- 旧 attempt metadata 没有 `roleProcesses` 或没有 `assistantText` 时继续按旧结构返回，不需要迁移
...
| GET | `/v1/team/task-runs/:runId/tasks/:taskId/attempts` | 读取 Task run 的 attempt metadata，包含可选 `roleProcesses.worker` / `roleProcesses.checker`；role process 可带 `assistantText.content` |
...
| `src/team/task-run-process-recorder.ts` | Canvas Task role process recorder：复用主 chat process 投影、节流写入 attempt `roleProcesses` 和 `assistantText`、取消 / 失败 / 完成 flush |
