...
- Team Console 底部 Dock 已优化为根节点托盘：每个 Dock item 按 kind 带左侧色条 glyph（Agent 绿 A、Task 琥珀 T、Source 青 S）和 title/meta 信息，固定宽度避免 hover 抖动，hover 时 `translateY(-4px)` 上浮；drop active 态有 inset guide 视觉；flight 动画补充节点 title；支持 `prefers-reduced-motion` 降级。
