...
- 点击 Task 菜单里的”最近运行”或”运行中”摘要会打开右侧 Run 观察节点；节点展开为多个独立节点——Run 状态节点展示运行阶段、耗时、attempt 数和进度消息，文件节点按 worker 输出、checker verdict、accepted result 分别展示文件名、路径和角色 runtime context / verdict 摘要；点击文件节点会在右侧展开第二级文件详情节点，根据文件扩展名使用安全渲染（JSON pretty print、Markdown 基础渲染、文本原样展示），不执行 HTML。SSE 观察流仍是后续后端能力，不在第一版里硬做。
