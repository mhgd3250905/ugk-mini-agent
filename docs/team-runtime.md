...
- Team Console preview 已加入 Agent Canvas MVP：通过 `GET /v1/agents` 读取主项目 Agent catalog，画布内同一 `agentId` 只能出现一次；点击 Agent 卡片进入 Focus Mode，上方保留当前 Agent 卡片，下方展开非 stream `POST /v1/agents/:agentId/chat` 聊天面板。该能力只引用现有 Agent profile，不创建 clone、instance、overlay 或画布局部技能安装，也不落地 WorkUnit / Plan 编排。
