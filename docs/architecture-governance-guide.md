...
这份文档是当前 native runtime 仓库的治理入口。旧版 Docker、云服务器、独立 Team Console dev server 和历史审计流水已清理；当前事实以代码、测试、`README.md`、`.env.native.example` 和各平台 native 文档为准。

## 先读顺序

| 任务 | 先读 |
| --- | --- |
| 全新接手 | `README.md`、`AGENTS.md`、`CONTRIBUTING.md`、`.env.native.example`、平台 native 文档 |
| Windows 本机部署 | `docs/native-windows-core.md`、`scripts/native-runtime-config.mjs`、`scripts/native-doctor-core.mjs` |
| macOS 本机部署 | `docs/native-macos.md`、`scripts/native-runtime-config.mjs`、`scripts/native-doctor-core.mjs` |
| Linux 本机部署 | `docs/native-linux.md`、`scripts/native-runtime-config.mjs`、`scripts/native-doctor-core.mjs` |
...
| `scripts/native-runtime-config.mjs` | Native runtime 环境拼装 | 写死端口、工具路径或数据目录 |
...
3. 是否影响外部用户 clone 后首次部署？如果影响，同步更新 `README.md`、`.env.native.example` 或对应平台 native 文档。
...
- 不要让外部用户必须安装 Docker 才能使用 native runtime 默认路径。
