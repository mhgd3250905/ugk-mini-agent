...
- 当前关键提交：
  - `80366d2 Show Conn Team Group start diagnostics`
  - `6ce2595 Add Team Console Group membership editing`
  - `50df295 Allow mutable Team Groups with run snapshots`
  - `282238a Align GroupRun completion with group pipeline`
  - `ca76c9f Polish Team Console Discovery history UI`
...
- 截至本次收口只读浏览器验证，`/v1/team/task-groups` 返回该 Group 为 `status="invalid"`，5174 Live 画布显示该 Group 可见、locked、`3 Tasks`、运行按钮 disabled，title 为后端 validation message：`Group task task_e1846fa41c83 connects to outside task task_8dc366711f37`。
- `/playground/conn` 新界面只读验证确认：新建 Conn 切到 Team Group 后，选项 `Group 1 · group_68c7cb331d7b（不可运行）`，`disabled=true`，未保存、未 POST/PATCH。
...
- Browser `http://127.0.0.1:5174/`：源码模块含 `onToggleTaskGroupLock`、`lockedTaskGroupNodeIdSet`、`data-task-group-locked` 和 membership controls；Live Group 可见，invalid/locked 状态下运行按钮禁用。
