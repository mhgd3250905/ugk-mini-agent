...
- 已改：Agent / Task / Source 根卡片按信息类型重排为身份区、内容区和底部 I/O 区；卡片内不再显示“收”按钮，也不再使用右侧 action rail 切割。收纳统一通过拖入底部 Dock 完成；Task dependency handle 仍保留在 Task 卡片右侧中部。Agent 根卡片高度为 `132px`，用于保留底部 model/browser binding 的内边距，不要再降回 `112px`。
