...
- 用户消息 meta 保持右对齐，但视觉顺序是时间在左、`YOU` 标签在右；不要再让 `YOU` 出现在时间左侧，否则右对齐气泡的身份锚点会反。
...
- 当前整体视觉基调已从偏冷蓝电子夜景收口为“静态工作台”：深色主题使用哑光 `#070a12` 近黑底、极淡网格和低对比工作画布色块；浅色主题使用冷灰蓝底、淡网格和浅蓝灰画布色块，避免页面白得空。
...
- 桌面端深色主题使用近黑静态网格、左侧会话索引、右侧深色工作画布和低强度冷色状态线；浅色主题使用冷灰蓝静态网格、白色会话索引、浅色工作画布和蓝灰状态线。浅色主题必须覆盖桌面背景层，不能让深色底或高对比网格漏进来把页面染脏。
...
- 用户消息 `message-meta` 显示时间和 `YOU`，整体贴右展示；视觉顺序保持时间在左、`YOU` 在右。
