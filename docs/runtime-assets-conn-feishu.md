...
- 空 Group 或语义 invalid Group 可以被保存和选中，但真正启动 `team_group` Conn 时会由 GroupRun start 返回 400 `invalid task group` 并把 ConnRun 记为 failed；这不是 409 active guard，也不应被记为 skipped success。
