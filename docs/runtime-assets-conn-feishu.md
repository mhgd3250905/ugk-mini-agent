...
- `team_group` Conn worker 启动 GroupRun 时调用 `POST /v1/team/task-groups/:groupId/runs`，请求没有业务 body 时不能携带 `content-type: application/json`；否则 Fastify 会在进入 route 前把空 body 判成 `400 Bad Request`。如果本地真实 UI 触发后 ConnRun 显示 `Team GroupRun start failed with 400: Bad Request`，先确认 `ugk-pi-conn-worker` 镜像是否已经重建并包含 `TeamGroupConnRunner.fetchJson()` 的空体 content-type 保护，别上来怀疑 Group contract。
- Team Group definition 允许变成 empty 或语义 invalid，但 Conn editor 新建 / 编辑 `team_group` Conn 时仍只允许选择可运行 Group；已保存 Conn 指向的 Group 后续变 invalid 时，真正启动会由 GroupRun start 返回 400 `invalid task group` 并把 ConnRun 记为 failed。这不是 409 active guard，也不应被记为 skipped success。失败 run 会保留 `resolvedSnapshot.executionType="team_group"`、`groupId`、`groupRunStartStatus` 和 `groupRunStartError`，因此 `/playground/conn` 与 `/playground` Conn manager 的 run detail 即使没有 `groupRunId`，也能显示 Team Group、start status/error 和 Group JSON 链接。
...
- 当 `conn.execution.type` 是 `team_group` 时，后台 worker 不启动 BackgroundAgentRunner，而是通过主服务 GroupRun API 启动并轮询 Team Task GroupRun：`POST /v1/team/task-groups/:groupId/runs`、`GET /v1/team/task-group-runs/:groupRunId`。GroupRun start 会硬拒绝 empty/invalid Group 并返回 400，ConnRun 记 failed；GroupRun 终态会映射到 conn run 终态，`completed` 记 succeeded，`completed_with_failures` / `failed` 记 failed，`cancelled` 记 cancelled。GroupRun 自身的 `completed` 只表示 Group 内真实 Task 流水线和内部边交付已完成；Discovery generated child run 失败仍保留在 `observedRuns` 诊断里，但不再单独把 GroupRun / ConnRun 拖成失败。
- `/playground/conn` 和 `/playground` 的 Conn run detail 会在 Team Group snapshot 里显示同源诊断链接：有 `groupId` 才显示 `/v1/team/task-groups/<encoded groupId>`，有 `groupRunId` 才显示 `/v1/team/task-group-runs/<encoded groupRunId>`；链接只读取既有 JSON endpoint，不代表新增后端 API。
- Team Group active guard 返回 409 时，conn run 会记为 succeeded skipped，summary 以 `Skipped:` 开头。invalid/empty Group start 必须返回 400 并作为 failed ConnRun 处理，不能复用 409 skipped 语义。conn run abort/cancel 且已创建 GroupRun 时，worker 会 best-effort 调 `POST /v1/team/task-group-runs/:groupRunId/cancel`；取消失败只作为事件记录，不覆盖原始取消语义。
