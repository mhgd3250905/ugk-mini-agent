...
整体布局遵循 Task → 菜单 → 二级节点（编辑/leader chat/run observer/文件详情）的层级关系。多个 Task 的菜单可以同时展开，每个 Task 的编辑、Leader chat 和 Run observer 作为按 Task 独立的二级 panel 同时存在，互不影响。点击”编辑”或”对话 Leader”不会替换一级菜单，也不会关闭其他 Task 的二级节点，而是在对应菜单右侧展开二级编辑节点或 leader Agent iframe 节点，连接线使用 fixed right-middle 到 left-middle 锚点；对话 Leader 复用 Agent 分支卡片的 header、iframe、右下角 resize handle、最大化按钮和标题栏双击最大化 / 还原，URL 形如 `/playground?view=chat&agentId=<leaderAgentId>&embed=team-console&teamTaskId=<taskId>&teamTaskMode=edit`；observer 文件详情面板也支持标题栏双击最大化 / 还原，左键按住标题栏拖动仍保持原行为。Run observer panel id 按 `branch.nodeId` 稳定命名（`run-observer-${branch.nodeId}`），不因打开数量变化而切换 id，确保布局状态（`panelPositionOverrides` / `panelMeasuredHeights` / `panelSizeOverrides`）在打开新 observer 时不会丢失。编辑草稿（`taskEditDraftByTaskId`）、保存中状态（`taskEditSavingByTaskId`）和冲突警告（`taskEditWarningByTaskId`）按 `taskId` 隔离，多个编辑节点同时打开时互不覆盖。删除会二次确认并调用 `POST /v1/team/tasks/:taskId/archive` 做软归档，成功后重新请求 `GET /v1/team/tasks` 并关闭分支。Agent / Task / Source 根卡片可通过右下角收纳按钮缩入画布左侧 Hub；收纳时根卡片、对应分支和连接线不占用画布空间，点击 Hub 条目会把根节点复原，并恢复收纳前保留的展开状态。

Agent / Task / Source 根卡片右下角除收纳按钮外，还各带一个清理入口。Source 根卡片和 Task 根卡片的清理按钮调用”归档”，点击后弹出自定义确认 modal（`role="dialog"`、`aria-modal="true"`），包含对象名称、影响说明、”取消”和”确认归档”按钮；确认后分别调用 `POST /v1/team/source-nodes/:sourceNodeId/archive` 和 `POST /v1/team/tasks/:taskId/archive`，归档成功后根卡片、Hub 条目、相关连接线、展开分支和本地 UI 状态同步清理，刷新页面后不会恢复已归档的 Task / Source。Agent 根卡片的清理按钮调用”移除”，确认后只从 Team Console 画布移除本地引用（canvas 节点、Hub 条目、展开分支和 localStorage 位置），不会调用 Agent profile archive API，真实 Agent profile 不受影响。归档失败时 modal 保持打开并在页面顶部 error banner 显示错误信息；Escape 键可在非保存中关闭 modal。
