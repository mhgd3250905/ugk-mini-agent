...
Agent / Task / Source 根卡片右下角除收纳按钮外，还各带一个清理入口。Source 根卡片和 Task 根卡片的清理按钮调用”归档”，点击后弹出二次确认（”确认归档”），确认后分别调用 `POST /v1/team/source-nodes/:sourceNodeId/archive` 和 `POST /v1/team/tasks/:taskId/archive`，归档成功后根卡片、Hub 条目、相关连接线、展开分支和本地 UI 状态同步清理，刷新页面后不会恢复已归档的 Task / Source。Agent 根卡片的清理按钮调用”移除”，确认后只从 Team Console 画布移除本地引用（canvas 节点、Hub 条目、展开分支和 localStorage 位置），不会调用 Agent profile archive API，真实 Agent profile 不受影响。归档失败时节点保留并在顶部 error banner 显示错误信息。

打开 Leader 对话分支时，面板顶部会显示“当前 Task 上下文”只读区域，列出 taskId、title、status、agents、input text、input/output ports、output contract、acceptance rules、teamTaskMode 和 teamTaskId；“复制 Task 上下文”按钮可将该上下文复制为格式化纯文本到剪贴板，方便粘贴到 Leader 对话中；剪贴板写入失败时可见文本仍可直接手动选择复制。Team Console 仍不解析 iframe 聊天文本，仍不自动更新 Task 定义。
