...
Live API 模式下 Group definition 始终来自后端 `GET /v1/team/task-groups`；画布“创建 Group”会向 `POST /v1/team/task-groups` 发送真实 `taskIds`，展开 Group frame 的“命名”会调用 `PATCH /v1/team/task-groups/:groupId` 更新同一个后端 `title`，移除 Group 会调用 `POST /v1/team/task-groups/:groupId/archive` 软归档，不删除 Task、connections 或 Task runs。`canvas-ui-state` 只保存 Group 展示态 `taskGroupDisplayStates: Array<{ groupId, collapsed, locked }>`，旧 live `taskGroups[].taskNodeIds` 只作为展示态迁移来源，不再作为 membership 权威数据。Live backend Group 的展开 frame 会读取最新 GroupRun，提供手动“运行”和 active GroupRun “终止”按钮：运行调用 `POST /v1/team/task-groups/:groupId/runs`，终止调用 `POST /v1/team/task-group-runs/:groupRunId/cancel`；active GroupRun 会轻量轮询详情，并在启动、终止或进入终态后 silent refresh 内部 Task run summary。Group 内已有 active Task run 时，Group 运行按钮禁用并显示“内部运行中”。Mock 模式仍可保留 UI-only Group fixture，不调用 GroupRun endpoints。Conn manager 和 `/playground/conn` 独立页已经可以选择后端 Group 作为 `execution.type="team_group"` 的执行对象；Team Console 不把 GroupRun 合进 `GET /v1/team/console/root-summary`。
