...
Execution Atlas 顶部工具栏按 command deck 收口：左侧是筛选、添加和统计，右侧是 viewport 控制；缩放只走固定可读档位 `45 / 50 / 67 / 75 / 90 / 100 / 110 / 125 / 150 / 180%`，画布平移 offset 会按当前 `devicePixelRatio` 对齐到设备像素，降低 DOM `transform: scale()` 后文字发虚和半像素抖动。多 Task 菜单的删除确认按具体 branch `nodeId` 隔离，不再被最后聚焦的 Task 菜单串台；根节点拖入垃圾桶后如果在确认 modal 里选择“取消”，Agent / Task / Source 会回滚到拖拽前坐标，Task 子树也同步回滚。
