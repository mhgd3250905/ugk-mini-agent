...
Task 操作菜单里的“运行记录”打开右侧抽屉，只按当前 Task 读取 `GET /v1/team/tasks/:taskId/run-history` 的轻量历史 summary。默认隐藏软归档 run，可切换显示；标记“最佳”通过 `PATCH /v1/team/task-runs/:runId/annotation` 写入 run annotation，并保证同一 Task 只有一个 best。点击某条 run 后才复用既有 attempts/files 接口懒加载过程摘要和文件预览，主 Atlas 画布不会渲染全部历史 attempts/files。运行记录行可点“装载此记录”/“取消装载”，Team Console 只在 `canvas-ui-state` 中保存 `{ taskId, runId }` 引用；同一 Task 有 active run 时历史装载标记显示为活跃 run 优先，不会伪装成当前执行上下文。本阶段“运行”仍按原请求启动 Task，不发送 `upstreamRunSelections`。第一版“删除”语义是软归档，不删除 `.data/team/task-runs/runs/<runId>` 下的真实 run 目录。
