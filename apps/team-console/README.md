...
- `GET /v1/team/tasks/:taskId/discovery-channel-sets`
- `POST /v1/team/tasks/:taskId/discovery-channel-sets`
- `PATCH /v1/team/tasks/:taskId/discovery-channel-sets/:channelSetId`
- `POST /v1/team/tasks/:taskId/discovery-channel-sets/:channelSetId/archive`
...
- `GET /v1/team/task-groups/:groupId/runs`
...
Task 操作菜单里的“运行记录”打开右侧抽屉，只按当前 Task 读取 `GET /v1/team/tasks/:taskId/run-history` 的轻量历史 summary。默认隐藏软归档 run，可切换显示；标记“最佳”通过 `PATCH /v1/team/task-runs/:runId/annotation` 写入 run annotation，并保证同一 Task 只有一个 best。点击某条 run 后才复用既有 attempts/files 接口懒加载过程摘要和文件预览，主 Atlas 画布不会渲染全部历史 attempts/files。运行记录行可点“装载此记录”/“取消装载”，Team Console 只在 `canvas-ui-state` 中保存 `{ taskId, runId }` 引用；同一 Task 有 active run 时历史装载标记显示为活跃 run 优先，不会伪装成当前执行上下文。启动下游 Task 时，Team Console 会检查指向该 Task 的非 stale typed task connection；如果上游 Task 已装载 historical run、同一上游 Task 当前没有 active run，且前端未从当前内存态判定该 loaded run 为非 `completed`，请求体会携带 `upstreamRunSelections: [{ connectionId, fromRunId }]`；从持久化 UI state 恢复后只有 `{ taskId, runId }`、状态未知的 selection 交由后端最终校验。没有有效 loaded upstream selection 时保持旧的普通 run 请求，不补最新 run、不查询历史、不读取旧 asset。第一版“删除”语义是软归档，不删除 `.data/team/task-runs/runs/<runId>` 下的真实 run 目录。

运行记录抽屉的每条历史行是状态化卡片，带 `data-run-status`；当前选中 run 使用 `aria-current="true"` 和 selected 高亮。Discovery 子画布中点击 generated child 打开运行记录时，来源 card 会保留 `is-history-open` 高亮，running / busy 执行态统一使用蓝青色，橙红色只保留给危险、失败或警告语义。
...
Discovery 子画布默认只在主 generated grid 渲染 active generated Tasks；`stale` generated Tasks 仍保留在 API summary、root Discovery 卡片计数和诊断里，但默认折叠为“旧项”入口，不参与主 grid 的 visible/total、running/queued/done 口径。用户点击“显示 N 个旧项”后才展开单独的 stale lane，用于检查旧 catalog 残留、reset-to-managed 或归档；这不会把 stale child 放回 root canvas，也不会改变后端 upsert / stale marking 合同。

Discovery 子画布 generated child card 的操作入口是卡片悬浮时显示的纵向菜单按钮。点击按钮后，菜单固定在按钮下方弹出，并允许超出子画布边界显示，避免靠近左右或底部边缘时被裁剪；菜单项包含编辑、归档和运行，运行记录通过点击 generated card 本体开关，不在 generated item 菜单里重复出现。generated Task 浅编辑面板只允许改 Task 名称和 Leader / Worker / Checker Agent，面板高度按内容自适应，表单内部不再单独滚动。
...
Live API 模式下 Task / Source 卡片拖动位置会写入浏览器 `localStorage` 并在刷新后恢复，但只保存 id 和画布坐标，不保存 WorkUnit 内容、`leaderAgentId`、`workerAgentId`、`checkerAgentId`、Source 内容或 Task run 定义。Team Console 另用 `canvas-ui-state` 保存 viewport、展开分支、Dock 收纳 id、segmented filter 选择和轻量历史 run 装载引用 `loadedTaskRunSelections: Array<{ taskId, runId }>`；恢复时会按当前 Agent / Task / Source catalog 清理过期 id。Task 定义始终以后端 `GET /v1/team/tasks` 返回为准；Source 定义始终以后端 `GET /v1/team/source-nodes` 返回为准；Task run 状态始终以后端 `GET /v1/team/tasks/:taskId/runs` / `GET /v1/team/task-runs/:runId` 返回为准。

Live API 模式下 Group definition 始终来自后端 `GET /v1/team/task-groups`；画布“创建 Group”会向 `POST /v1/team/task-groups` 发送真实 `taskIds`，展开 Group frame 的“命名”会调用 `PATCH /v1/team/task-groups/:groupId` 更新同一个后端 `title`，移除 Group 会调用 `POST /v1/team/task-groups/:groupId/archive` 软归档，不删除 Task、connections 或 Task runs。`canvas-ui-state` 只保存 Group 展示态 `taskGroupDisplayStates: Array<{ groupId, collapsed, locked }>`，旧 live `taskGroups[].taskNodeIds` 只作为展示态迁移来源，不再作为 membership 权威数据。Discovery 子画布渠道集选择支持“全选有效项 / 取消全选”，只选择 active generated Tasks，不把 `stale hidden` 旧项带入渠道集。Live backend Group 的展开 frame 会读取最新 GroupRun，提供手动“运行”和 active GroupRun “终止”按钮：运行调用 `POST /v1/team/task-groups/:groupId/runs`，终止调用 `POST /v1/team/task-group-runs/:groupRunId/cancel`；active GroupRun 会轻量轮询详情，并在启动、终止或进入终态后 silent refresh 内部 Task run summary。Group 内已有 active Task run 时，Group 运行按钮禁用并显示“内部运行中”。Mock 模式仍可保留 UI-only Group fixture，不调用 GroupRun endpoints。Conn manager 和 `/playground/conn` 独立页已经可以选择后端 Group 作为 `execution.type="team_group"` 的执行对象；Team Console 不把 GroupRun 合进 `GET /v1/team/console/root-summary`。
