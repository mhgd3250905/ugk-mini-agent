...
- 三类 active 连接线（typed Task connection、Source connection、control dependency）均可通过线段中点的切断按钮直接删除；按钮颜色随连接线类型（绿色/青色/琥珀色），`aria-label` 包含源和目标名称，点击后直接调用对应 DELETE API，失败时保留原线并显示 error banner
