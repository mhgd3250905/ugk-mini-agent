...
Control dependency 是 typed connection 之外的另一条边，只表达"Task A 成功完成后自动启动 Task B"，不传数据、不要求 `inputPorts` / `outputPorts`、不生成 typed artifact、不写 `boundInputs`。无 ports 的 Task 也可以通过 control dependency 串行运行。用户在 Task 卡片右侧中部的 amber 圆形 dependency handle（区别于 typed port chip）点击选择 source Task，再点击 target Task 的 handle 完成创建；handle 有 `aria-label` 区分"设为依赖源"、"设为依赖目标"和"已选依赖源"三种状态。Execution Atlas 会渲染一条 dashed amber 线（`.emap-link-task-dependency`）。后端 API 为 `GET /v1/team/task-dependencies`、`POST /v1/team/task-dependencies`、`DELETE /v1/team/task-dependencies/:dependencyId`，持久化为 `.data/team/task-dependencies.json`。Cycle 防护覆盖 typed connections + control dependencies 的混合图。Stale dependency（source 或 target Task 缺失 / 归档）不渲染线、不触发下游。Mock 模式下 dependency handle 可正常交互。
