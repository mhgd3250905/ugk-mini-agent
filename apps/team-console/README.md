...
`$BASE_URL` 来自 `.env.native` / `.env.native.example` 中的 `PUBLIC_BASE_URL`、`HOST`、`PORT`。

## 运行方式

根目录 `native:start` 会先构建 Team Console，然后启动主服务、Team worker 和 Conn worker：
...
构建产物写入 `apps/team-console/dist`，由主服务 `/playground/team` 同源路由提供。
