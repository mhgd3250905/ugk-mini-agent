...
Task Run observer 也渲染 `Worker 过程` / `Checker 过程` 两个独立 canvas branch node。前端消费 additive contract `attempt.roleProcesses.worker` / `attempt.roleProcesses.checker`，节点顶部按优先级展示：(1) `assistantText.content`（Agent 自述 / 推理文本，`formatAssistantText()` 保留换行、中文标点自然断句、每行独立 `<p>`，最多 5 行超限显示”已隐藏 X 行”，单行超过 200 字符会截断并显示”已截断 X 长行”，`max-height: 172px` 内部滚动），(2) `assistantText` 缺失时的 current action + 最新 narration 回退，(3) tool groups 作为可折叠证据区。下半部每个过程节点只渲染 1 个最相关 tool group（优先 active / running，其次最新 finished / error），其余 group 显示”仅显示最近 1 组，已隐藏 X 组 / X 条”；tool group 内仍按 `toolCallId` 分组、可折叠 / 展开。完整过程数据仍来自后端 attempt metadata，前端不丢数据，只做 DOM 渲染限流。Live API 后端缺少 `roleProcesses` 时会显示等待过程数据，role process 为 `null` 或条目为空时显示暂无过程条目，不报错。运行中的 observer 不渲染空文件占位节点，不显示 `正在刷新...` / `最后刷新` 这类随轮询变化的刷新元信息，active run 轮询的瞬时连接失败也不插入红色错误节点，以保留现有画面稳定性；终态 run 的读取失败仍显示错误。过程节点复用现有 Task 操作树拖动语义；拖动 Task 根节点、菜单节点、Run observer 子节点或 resize 文件详情时，会暂停 Task branch / child panel 的自动高度测量，避免运行中轮询刷新强制 layout 导致节点拖动卡顿或闪烁。Run observer 当前仍通过现有 run state / attempt metadata / attempt file API 轮询，不接 SSE，也不新增 endpoint。
