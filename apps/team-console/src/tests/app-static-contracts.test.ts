...
describe("Team Console static contracts", () => {
  ...
  it("exposes Dell 1996 as an independent persisted visual theme", () => {
    ...
    expect(mapCss).toMatch(/\[data-visual-theme="dell-1996"\] \.emap-task-agent-row,[\s\S]*?\.emap-discovery-summary-row span[\s\S]*?{[^}]*box-shadow:\s*none;/s);
    expect(mapCss).toMatch(/\[data-visual-theme="dell-1996"\] \.emap-task-port-chip:hover,[\s\S]*?\.emap-task-agent-row:hover,[\s\S]*?\.emap-discovery-summary-row span:hover\s*{[^}]*box-shadow:\s*3px 3px 0 var\(--dell-shadow\);/s);
    expect(mapCss).toMatch(/\[data-visual-theme="dell-1996"\] \.emap-task-port-chip:hover,[\s\S]*?\.emap-task-agent-row:hover,[\s\S]*?\.emap-discovery-summary-row span:hover\s*{[^}]*transform:\s*translate\(-1px,\s*-1px\);/s);
