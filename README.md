...
| lsof | Linux 自动清理端口占用需要 |

首次启动后，打开 `/playground/model-sources` 添加模型 API 源。新安装不会自带 provider 或 API key。

### Windows

Windows 是当前稳定基线。推荐直接使用仓库根目录的启动器：
...
深度说明见 [Windows native guide](docs/native-windows-core.md)。
...
本项目已经在 Alibaba Cloud Linux 3 上完成实机部署验证。推荐使用较新的发行版，例如 Alibaba Cloud Linux 3、Ubuntu 22.04/24.04、Debian 12、Rocky Linux 9。
...
安装系统依赖。Alibaba Cloud Linux / RHEL / Fedora 系：
...
如果系统自带 `python3` 仍是旧版本，例如 `3.6.x`，不要覆盖系统 Python。给当前用户加一个 PATH 优先级 shim：
...
深度说明见 [Linux native guide](docs/native-linux.md)。
