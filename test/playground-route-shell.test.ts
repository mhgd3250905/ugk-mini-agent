...
test("GET /playground returns the test UI html", async () => {
	...
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*position:\s*fixed;/);
	...
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*top:\s*auto;/);
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*bottom:\s*calc\(var\(--command-deck-toast-offset, var\(--command-deck-offset, 96px\)\) \+ 12px\);/);
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*left:\s*var\(--command-deck-center-x, 50%\);/);
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*transform:\s*translateX\(-50%\);/);
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*width:\s*fit-content;/);
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*min-width:\s*220px;/);
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*max-width:\s*min\(560px, calc\(100vw - 32px\)\);/);
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*max-height:\s*min\(160px, calc\(100vh - 32px\)\);/);
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*overflow:\s*auto;/);
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*border:\s*1px solid rgba\(255, 113, 136, 0\.28\);/);
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*border-radius:\s*10px;/);
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*background:\s*rgba\(47, 17, 25, 0\.96\);/);
	assert.match(response.body, /\.error-banner\s*\{[\s\S]*z-index:\s*1000;/);
	assert.doesNotMatch(response.body, /\.chat-stage > \.error-banner\s*\{/);
	...
	assert.match(response.body, /\.error-banner-close\s*\{[\s\S]*border-radius:\s*8px;/);
	assert.match(response.body, /\.error-banner-close\s*\{[\s\S]*border:\s*1px solid rgba\(255, 232, 237, 0\.14\);/);
	assert.match(response.body, /\.error-banner-close\s*\{[\s\S]*background:\s*rgba\(90, 34, 48, 0\.62\);/);
	...
	assert.doesNotMatch(response.body, /id="runtime-summary"/);
	assert.doesNotMatch(response.body, /id="runtime-model-value"/);
	assert.doesNotMatch(response.body, /id="runtime-browser-value"/);
	assert.doesNotMatch(response.body, /function renderRuntimeSummary\(\)/);
	assert.doesNotMatch(response.body, /function syncRuntimeSummary\(\)/);
	assert.doesNotMatch(response.body, /当前 API 源/);
	assert.doesNotMatch(response.body, /当前 Chrome/);
