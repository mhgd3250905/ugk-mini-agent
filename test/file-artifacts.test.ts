...
test("buildPromptWithAssetContext falls back to the native default public base URL", () => {
	...
	try {
		...
		assert.match(prompt, new RegExp(`Current user-facing base URL: ${escapeRegExp(defaultNativeEnv.PUBLIC_BASE_URL ?? "")}\\.`));
...
test("rewriteUserVisibleLocalArtifactLinks converts supported container file paths for host-visible text", () => {
	...
	assert.equal(
		rewritten,
		`打开 ${defaultNativeEnv.PUBLIC_BASE_URL}/v1/local-file?path=%2Fapp%2Fpublic%2Fzhihu-hot-share.html，然后看 ${defaultNativeEnv.PUBLIC_BASE_URL}/v1/local-file?path=%2Fapp%2Fruntime%2Freport-medtrum-v2.html。`,
	);
});
