...
test("playground renders agent management entry points and workspace", () => {
	...
	assert.doesNotMatch(html, /id="agent-editor-browser-select"/);
	assert.doesNotMatch(html, /fetch\("\/v1\/browsers"/);
	assert.doesNotMatch(html, /defaultBrowserId/);
	assert.doesNotMatch(html, /renderBrowserOptions/);
	assert.doesNotMatch(html, /confirmAgentBrowserChangeIfNeeded/);
	...
	assert.doesNotMatch(html, /默认浏览器/);
